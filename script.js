...
let scores = {};
const ORDER_FORM_URL = "https://forms.office.com/Pages/ResponsePage.aspx?id=SJPOFSq-K0aPwOF2WpsgShjBS2QFxkxAu1LxWWFlBgtUNEFXUlA1TUpEUkRLSjhVVTdXVFowTFk5Ny4u";
...
function renderQuestion() {
  ...
  question.options.forEach(function (option) {
    ...
    // 这就是你问的第四步：点击哪个选项，就给对应人格 +1
    button.onclick = function () {
      selectOption(option.type, option.weight);
    };
...
function selectOption(type, weight = 1) {
  const fixedType = normalizeType(type);

  if (scores[fixedType] === undefined) {
    scores[fixedType] = 0;
  }

  scores[fixedType] += weight;
